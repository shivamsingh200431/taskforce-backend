...
- Current workload counts pending/approved/overdue work and excludes completed, rejected, and inactive chores.
...
- `src/server.js` — mount the recurring-template routes without changing existing chore route behavior.
- `package.json` — add a test command only if required by the existing test layout; do not add a new runtime dependency unless implementation demonstrates it is necessary.
...
- `src/controllers/recurringChoreController.js` — HTTP handlers for template CRUD and optional manual generation.
...
- `tests/controllers/recurringChoreController.test.js` — recurring API permission and request behavior if the repository's existing test harness supports controller tests.
...
Add `recurringTemplateId` as an ObjectId reference to `RecurringChoreTemplate`, `occurrenceDate` as a required Date for recurring instances, and `generationType` using the existing `GENERATION_TYPE` values. Add the unique compound index `{ recurringTemplateId: 1, occurrenceDate: 1 }`. Keep ordinary one-time chores compatible by allowing recurring fields to be null when `choreType` is `one-time`. Add `processingLeaseUntil` with a default of null under scheduler metadata.
...
- Modify: `src/constants/chore.constants.js` only if an existing constant must be exposed cleanly

**Interfaces:**
- `getCurrentWorkload(userId, householdId)` returns the user's current workload using active pending/approved/overdue chores and difficulty points.
...
Cover: a pending difficulty-3 chore contributes 30 points; completed/rejected chores contribute nothing; overdue chores contribute to current workload; inactive historical work does not count; recent assignments are counted only inside the requested window; recent difficulty uses the same window; users with no matching chores return zeroes.
...
Use `Chore` queries scoped by both `householdId` and `assignedTo`. Current workload must include the statuses that represent outstanding work in the current schema (`pending`, `approved`, `overdue` where represented) and exclude completed/rejected/inactive records. Sum difficulty through the existing difficulty-point mapping, treating a missing difficulty as zero for the workload calculation. Recent burden queries must use `createdAt`/assignment-related historical records available in the current schema and the supplied `windowStart`.
...
```bash
git add src/services/workload.service.js tests/services/workload.service.test.js src/constants/chore.constants.js
...
Query `Membership` for household members and use membership records as the eligibility boundary. Fixed assignment must verify the configured user has a membership in the template household. Rotation candidates must be active household members represented by membership records.
...
- `POST /api/recurring-chores/:id/generate` optionally performs manual generation through `generateOccurrence`/`processTemplate` with `generationType = manual`.
...
**Files:**
- Modify: `src/server.js`
- Test: `tests/services/recurringScheduler.service.test.js` if startup behavior needs coverage
- Modify: `package.json` only if a dedicated test/startup command is required by the existing setup

**Interfaces:**
- Application startup begins the recurring scheduler loop after database initialization.
- The scheduler loop calls `runSchedulerTick` at a fixed interval and never overlaps its own tick execution.

- [ ] **Step 1: Write a failing startup/scheduler-loop test**

Test that a scheduler tick is invoked at the configured interval and that a slow tick prevents a second overlapping tick from starting.
...
Start the loop only after the application has connected to MongoDB. Guard the loop with an in-flight flag so one application instance does not run overlapping ticks. Keep the interval configurable and do not expose scheduler execution as an HTTP endpoint.

- [ ] **Step 4: Run startup and service tests**
...
```bash
git add src/server.js package.json tests/services/recurringScheduler.service.test.js
